...
exit_node_key = utils.connect_with_server("http://10.0.0.15:5000","exit")
print("key - - - - " , exit_node_key)
sender_info = ""
...
def node(node_type):
    global sender_info
    global exit_node_key
    data = request.data
    sender_info = request.headers.get('Sender-Info')
    sender_info = "http://" + str(sender_info) + "/back_way"
    print("sender_info - - - - - " , sender_info)
    if node_type == 'exit':
        headers = {"Sender-Info": "127.0.0.1:5003"}
        answer = utils.move_package_and_remove_encrepion(exit_node_key,data,headers)
        if answer != "":
            response = requests.get(answer).text
            print(response)
            utils.move_package_back_and_add_encrepion(exit_node_key, response, sender_info)
...
def back_way():
    ...
    print("12345678")
    data = request.data
    utils.move_package_back_and_add_encrepion(exit_node_key, data, sender_info)
    return "1"
